...
with st.sidebar:
   ...
   measure = st.radio("Choose Measure:",['CASUALTIES','VEHICLES','ACCIDENTS','AVERAGE_AGE_OF_DRIVER','MALE_DRIVERS','FEMALE_DRIVERS'])

   values = st.slider('Select  H3 resolution',1,10,7)
...
def CARTO(R, S_YEAR, S_TOWN_CODE):
        # Filter ACCIDENTS_WITH_TOWN_CITY_DETAILS table based on TOWN_CODE
    ACCIDENT_TOWN_CITY = session.table('ACCIDENTS_WITH_TOWN_CITY_DETAILS').filter(F.col('TOWN_CODE') == S_TOWN_CODE)

        # Join ACCIDENTS and ACCIDENTS_WITH_TOWN_CITY_DETAILS tables based on ACCIDENT_INDEX
    ACCIDENT_DETAILS = ACCIDENTS.filter(F.col('ACCIDENT_YEAR') == S_YEAR).join(ACCIDENT_TOWN_CITY, 'ACCIDENT_INDEX')

        # Select necessary columns and apply UDFs
    df = ACCIDENT_DETAILS.select('LATITUDE', 'LONGITUDE', 'ACCIDENT_INDEX', 'ACCIDENT_YEAR') \
        .with_column('H3', F.call_udf('ANALYTICS_TOOLBOX.CARTO.H3_FROMLONGLAT', F.col('LONGITUDE'), F.col('LATITUDE'), R)) \
    ...
    vehicles = df.join(
        session.table('VEHICLES').select(F.when(F.col('SEX_OF_DRIVER') == 1, F.lit(1)).alias('MALES'),
            ...
            .group_by('H3').agg(F.any_value('WKT').alias('WKT'), F.count('ACCIDENT_INDEX').alias('VEHICLES'),
    ...
    all_data = accidents.join(casualties, 'H3', lsuffix='left') \
            .select('H3', F.col('WKT').alias('WKT2'), 'ACCIDENTS', 'CASUALTIES', 'FATAL', 'AVERAGE_AGE_OF_CASUALTY',
                'SEVERE', 'MINOR').join(vehicles, 'H3', 'LEFT') \
            .select('H3', 'WKT', 'ACCIDENTS', 'CASUALTIES', 'VEHICLES', 'FATAL', 'SEVERE', 'MINOR',
                'AVERAGE_AGE_OF_DRIVER', 'MALE_DRIVERS', 'FEMALE_DRIVERS')
...
with col20:
    ...
    selected_data = CARTO(values,S_YEAR, S_TOWN_CODE).sort_values(by=measure).head(1)

        

    if measure == 'AVERAGE_AGE_OF_DRIVER':
        TOTAL = CARTO(values,S_YEAR, S_TOWN_CODE)[measure].mean()
    else:
        TOTAL = CARTO(values,S_YEAR, S_TOWN_CODE)[measure].sum()
...
with col21:
    ...
    geodframe = gpd.GeoDataFrame(CARTO(values,S_YEAR, S_TOWN_CODE))
...
with col3:
    fig, ax = plt.subplots()
    d = CARTO(values,S_YEAR, S_TOWN_CODE)
    x = d[measure]
...
with col4:
    ...
    st.markdown(f'###### {measure} VS NUMBER OF VEHICLES')    
    st.pyplot(fig)
